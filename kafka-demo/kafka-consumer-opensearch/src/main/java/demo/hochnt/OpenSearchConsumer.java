        ...
        try (openSearchClient; kafkaConsumer) {
            ...
            while (true) {
                ...
                for (var record : records) {
                    // make idempotence
                    // strategy 1 - Define an ID using kafka record coordinator
                    // String id = record.topic() + "_" + record.partition() + "_" + record.offset(); // unique
                    try {
                        // strategy 2 - Extract id from metadata
                        String id2 = extractId(record.value());
                        log.info("meta id: {}", id2);
                        IndexRequest indexRequest = new IndexRequest("wikimedia")
                                .source(record.value(), XContentType.JSON)
                                .id(id2); //send json data to openSearch with id
                        IndexResponse response = openSearchClient.index(indexRequest, RequestOptions.DEFAULT);
                        log.info("Insert 1 doc into openSearch with response: {}", response.getId());
                        // GET /wikimedia/_doc/{response.getId()}
                    } catch (Exception e) {
                ...
                // at-least-once cuz only after we've successfully processed the entire batch (after for-loop)
